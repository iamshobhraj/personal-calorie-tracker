...
export const signup = (input: SignupInput) => apiRequest("/auth/signup", { method: "POST", body: input, schema: envelopeSchema(z.object({ userId: z.string().uuid(), status: z.literal("ACTIVE") })), authenticated: false, idempotencyKey: crypto.randomUUID() });
