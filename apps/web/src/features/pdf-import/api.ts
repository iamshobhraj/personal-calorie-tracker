...
export function commitPdfImport(importId: string, selectedRowIds: number[]) { return apiRequest(`/pdf-imports/${importId}/commit`, { method: "POST", body: { selectedRowIds }, schema: envelopeSchema(z.object({ importId: z.string().uuid(), status: z.literal("COMMITTED"), createdMealEntryIds: z.array(z.string().uuid()), createdCount: z.number() })), idempotencyKey: generateUuid() }); }
